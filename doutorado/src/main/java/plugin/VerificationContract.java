...
public class VerificationContract {
	...
	//	cspFDR =  cspFDR + "\n include \"auxiliar_" + instance_temp.getName() + ".csp\" \n";
		...
		cspFDR =  cspFDR +"\n--Condition A.2.1: Every channel in P is an I/O Channel"
                    + "\nassert not Test(inter(inputs_" + instance_temp.getName()+ ",outputs_" + instance_temp.getName() + ") == {}) [T= ERROR";
        
        cspFDR =   cspFDR +"\n --Assertions:\n\n--Condition A.2: I/O Process "
                + "\n\n--Condition A.2.2: The contract has infinite set of traces"
                + "\nassert not HideAll_" + instance_temp.getName() + ":[divergence free [FD]]"
                + "\n\n--Condition A.2.3:	The contract is divergence-free"
                + "\nassert " + instance_temp.getName() + ":[divergence free [FD]]"
                + "\n\n--Condition A.2.4: The contract is input deterministic"
                + "\nassert LHS_InputDet_" + instance_temp.getName() + " [F= RHS_InputDet_" + instance_temp.getName() + ""
                + "\n\n--Condition A.2.5: The contract is strong output decisive"
                + "\nassert LHS_OutputDec_A_" + instance_temp.getName() + " [F= RHS_OutputDec_A_" + instance_temp.getName() + "";
        //;
        //canais da instancia 
	     ...
	     //   cspFDR = cspFDR + "\n assert LHS_OutputDec_B_" + instance_temp.getName() + "(" + str_temp + ") [F= RHS_OutputDec_B_" + instance_temp.getName() + "(" + str_temp+ ")";
        }
        }
	...
	public void  cspAuxiliar(String name)  {
		...
		String str ="";
		try {
		arquivo =  new FileWriter(new File("C:/Users/flavi/eclipse-workspace/CorTeste/src/main/resources/"+"auxiliar_"+ name +".csp" ));  
		 ...
		 str =  "---------------------------------------------------------\n"
                ...
                + "---------------------------------------------------------\n"
                + "\n"
                + "events_" + name + " = union(inputs_" + name + ",outputs_" + name + ")\n"
                + "allInputs_" + name +  "= Union({ inputs_" + name + "})\n"
                + "allOutputs_" + name +  " = Union({ outputs_" + name + "})\n"
                + "-- Filters inputs or outputs from a given set of events\n"
                + "\n"
                + "filter_"+name +"(inout,S) =\n"
                + "	if inout == in then inter(allInputs_"+ name +",S) else inter(allOutputs_" + name +",S)\n"
                + "\n"
                + "\n"
                + "HideOutputs_" + name + " = " + name + " \\ outputs_" + name + "\n"
                + "HideInputs_" + name + "  = " + name + " \\ inputs_" + name + "\n"
                + "HideAll_" + name + " = " + name + " \\ union(outputs_" + name + ",inputs_" + name + ")\n"
                + "CollapseOutputs_" + name + " = " + name + " [[ x <- o | x <- outputs_" + name + " ]] \\ allInputs_"+ name +"\n"
                + "\n"
                + "-- Renaming I/O\n"
                + "R_IO_" + name + "(a, b) = seq({(a.x, b.x) | x <- extensions(a), member(a.x, outputs_" + name + ")})\n"
                + "inputs_R_IO_" + name + "(a,b)  = inputs_" + name + "\n"
                + "outputs_R_IO_" + name + "(a,b) = { b.x | x <- extensions(a), member(a.x, outputs_" + name + ") }\n"
                + "\n"
                + "\n"
                + "-- Protocol Implementation\n"
                + "inputs_PROT_IMP_" + name + "(c)  = inter(inputs_" + name + ",{|c|})\n"
                + "outputs_PROT_IMP_" + name + "(c) = inter(outputs_" + name + ",{|c|})\n"
                + "\n"
                + "-- Protocol Implementation and renaming\n"
                + "PROT_IMP_R_" + name + "(r)   = rename(" + name + ",r) --modifidado por sarah\n"
                + "inputs_PROT_IMP_R_" + name + "(e,r)   = replace(inputs_PROT_IMP_" + name + "(e), r)\n"
                + "outputs_PROT_IMP_R_" + name + "(e,r)   = replace(outputs_PROT_IMP_" + name + "(e), r)\n"
                + "\n"
                + "--InBufferProt(P,c) =  CIO(P[[ c.in.x  <- i, c.out.y <- o | x  <- extensions(c.in), y  <- extensions(c.out)]])\n"
                + "InBufferProt_" + name + "(c) =  CIO(" + name + "[[ x  <- in, y <- out | x  <- inputs_" + name + ", y  <- outputs_" + name + "]])\n"
                + "\n"
                + "-- Channels Projection\n"
                + "PROJECTION_" + name + "(cs) = " + name + " \\ (diff(Events, Union({ {| c |} | c <- cs})))\n"
                + "\n"
                + "\n"
                + "-- Protocol Implementation\n"
                + "PROT_IMP_def_" + name + "(c) = " + name + " \\ (diff(Events, {|c|}))\n"
                + "\n"
                ...
                + "-- Input Determinism\n"
                + "----------------------------------------\n"
                + "Clunking_" + name + " = " + name + " [| AllButClunk |] Clunker\n"
                + "RHS_InputDet_" + name + " = (Clunking_" + name + "[|{clunk}|]Clunking_" + name + ") \\ {clunk} [|AllButClunk|] Repeat\n"
                + "\n"
                + "LHS_InputDet_" + name + " = Deterministic(inputs_" + name + ")\n"
                + "\n"
                ...
                + "--    (Clunking_" + name + "[|diff(Events, outputs(P))|]Clunking_" + name + ") \\ {clunk} \n"
                + "--    [| AllButClunk |] \n"
                ...
                + "\n"
                + "One2Many_" + name + " = \n"
                + "    ([] x:diff(Events,union(outputs_" + name + ", {clunk})) @ x -> One2Many_" + name + ")\n"
                + "    [] ([] c:outputs_" + name + " @ [] x:{|c|} @ x -> One2Many_" + name + "'(c,x))\n"
                + "\n"
                + "One2Many_" + name + "'(c,x) = [] y:chan_" + name + "(c) @ y -> if x==y then One2Many_" + name + " else STOP\n"
                + "\n"
                + "RHS_OutputDec_A_" + name + " = \n"
                + "    (Clunking_" + name + "[|diff(Events, outputs_" + name + ")|]Clunking_" + name + ") \\ {clunk} \n"
                + "    [| AllButClunk |] \n"
                + "    One2Many_" + name + "\n"
                + "\n"
                + "-----------------------------------------------------------------\n"
                ...
                + "            STOP \n"
                + "            |~|\n"
                + "            ([] x:diff(Events,union(outputs_" + name + ", {clunk})) @ x -> LHS_OutputDec_A_" + name + ")\n"
                + "            [] \n"
                + "            ([] x:outputs_" + name + " @ x -> (|~| y:chan_" + name + "(x) @ y -> LHS_OutputDec_A_" + name + "))\n"
                + "\n"
                + "\n"
                + "----------------------------------------------------\n"
                ...
                + "SecondCopy_" + name + " = " + name + " [| AllButClunk |] clunk -> DoubleClunker\n"
                + "\n"
                + "LHS_OutputDec_B_" + name + "(c) = (FirstCopy_" +  name + "[|{clunk}|]SecondCopy_" + name + ")\\{clunk} [|Events|] LHS_Test(inter({|c|}, outputs_" + name + "))\n"
                + "\n"
                + "RHS_OutputDec_B_" + name + "(c) = (FirstCopy_" + name + "[|{clunk}|]SecondCopy_" + name + ")\\{clunk} [|Events|] RHS_Test(inter({|c|}, outputs_" + name + "))\n"
                + "\n"
                + chan;
	...
	public void functionAux() {
		...
		boolean exists = (new File("C:/Users/flavi/eclipse-workspace/CorTeste/src/main/resources/"+"function_aux.csp")).exists();
		 if(exists) {
			
		  System.out.println("existe");
		  }
		  {
			arquivo =  new FileWriter(new File("C:/Users/flavi/eclipse-workspace/CorTeste/src/main/resources/"+"function_aux.csp" ));  
	...
	public String chan( String name) {
		...
		/*        String chan = "-- Outputs events with same prefixing of a given output event\n" + 
                "\nchan_" + contrato.getName()  + "(ev) = \n" +
"    inter(outputs_" + contrato.getName()  + ",\n" +
"          Union({ ";
                for (int i = 0; i < contrato.getChannel().size(); i++){
                    if(i < contrato.getChannel().size()-1){
